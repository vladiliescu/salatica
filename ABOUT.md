...
**Panou de ingrediente pliabil** — panoul cu toate chips-urile e înalt (8 categorii), iar după ce ai ales devine doar un perete de derulat până la rezultate. De aceea, când ai o selecție, un buton „Restrânge ↑" jos în panou îl pliază într-o bară-sumar de un rând: „Ai N · listă de ingrediente · ✎ Editează" (numele se taie cu elipsă ca să rămână pe un rând). Bara-sumar înlocuiește și panoul, și contorul (sumarul arată deja numărul + resetarea, refolosind id-ul `clear-all`). Tap pe bară sau pe „Editează" redeschide panoul. *Nu* se pliază automat la selectare — ai nevoie să adaugi mai multe ingrediente fără ca panoul să se închidă după fiecare tap — deci pliarea e o acțiune explicită; resetarea readuce panoul deschis. Focusul se mută între „Restrânge" și „Editează" (controale pereche) la fiecare re-render. Vezi `renderIngredients` / `renderFridgeSummary` / `panelCollapsed`.
